...
public class ListeCoursesDAO implements Closeable {
    public static final String CREATE_QUERY = "CREATE TABLE liste_courses(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, listName VARCHAR(15) NOT NULL, listCat VARCHAR(20) NOT NULL, produit VARCHAR(15) NOT NULL, quantite INTEGER NOT NULL DEFAULT 0, uniteId INTEGER NOT NULL DEFAULT 0, categorieProdId1 INTEGER NOT NULL, categorieProdId2 INTEGER NOT NULL, categorieProdId3 INTEGER NOT NULL, achete INTEGER NOT NULL DEFAULT 0, CONSTRAINT fk_unites FOREIGN KEY (uniteId) REFERENCES unites(id), CONSTRAINT fk_categories1 FOREIGN KEY (categorieProdId1) REFERENCES categories(id), CONSTRAINT fk_categories2 FOREIGN KEY (categorieProdId2) REFERENCES categories(id), CONSTRAINT fk_categories3 FOREIGN KEY (categorieProdId3) REFERENCES categories(id))";
    ...
    public ListeCourses getListeCoursesfromCursor(Cursor cursor) {
        ...
        listeCourses.setProduit(cursor.getString(cursor.getColumnIndex("listName")));
        listeCourses.setProduit(cursor.getString(cursor.getColumnIndex("listCat")));
    ...
    public long insert(ListeCourses listeCourses) {
        ...
        cv.put("listName", listeCourses.getListName());
        cv.put("listCat", listeCourses.getListCat());
